...
namespace ClinicOS.UnitTests.Features.Accounts.AccountManagement.Commands.ActivateUser;
...
public class ActivateUserCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_Should_Return_Failure_When_Activation_Fails()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var error = new Error("User.ActivationFailed", "Could not activate user", ErrorType.Validation);

        _userServiceMock.Setup(u => u.ActivateUserAsync(userId, It.IsAny<CancellationToken>()))
